...
The current Wan backend owns descriptors advertising:

- text-to-video;
- image-to-video;
- both workflows for TI2V models.

A pure I2V checkpoint marks source image as required. A TI2V checkpoint keeps it
optional.
...
Model-specific defaults may override the family defaults. Turbo-named models,
for example, receive fast checkpoint defaults rather than creating a separate
application mode.
...
6. show audio/output capabilities without exposing backend names;
7. remove any remaining Wan-specific labels or engine settings.

No additional runtime family tabs should be introduced during that rewrite.

## Future Extension Rules

A new video architecture should normally require:
