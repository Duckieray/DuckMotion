"""Lightweight probes for DuckMotion isolated runtime interpreters."""
...
for module_name, symbol_name in requirements:
    try:
        ...
        out["ready"] = False
        out["missing"].append({"module": module_name, "symbol": symbol_name, "error": str(exc)})
...
'''
    try:
        ...
        return {"ready": False, "python": str(executable), "reason": f"Runtime probe failed to start: {exc}"}
