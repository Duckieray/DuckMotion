...
DuckMotion imports a pinned Comfy core checkout as a Python library. It does not
start ComfyUI or execute arbitrary companion JSON. The registered topology here
mirrors the current LTX-2.5 two-stage I2V subgraph while allow-listed companion
settings remain recipe data.
"""
...
from ltx_convrot_recipe import (
    ...
    DEFAULT_SAMPLER,
    DEFAULT_STAGE1_SAMPLER,
    DEFAULT_STAGE1_SIGMAS,
    DEFAULT_STAGE2_FIXED_SEED,
    DEFAULT_STAGE2_NOISE_POLICY,
    DEFAULT_STAGE2_SAMPLER,
...
# Kept as descriptive metadata/backward-compatible names. The native stage
# transition is the learned LTX x2 upscaler only; there is no generic 0.5x
# LatentUpscaleBy step after it.
LATENT_UPSCALE_METHOD = "ltx_learned_x2"
...
def _snap_dimension(value: int) -> int:
    # The two-stage recipe starts at half resolution. A 64px outer grid keeps
    # that half-resolution latent on LTX's 32px spatial grid.
...
def _stage2_seed(seed: int) -> int:
    """Derive the legacy incremented second noise stream reproducibly."""
    return (int(seed) + 1) & UINT64_MASK


def _resolved_stage2_seed(seed: int, recipe: dict) -> int:
    policy = str(recipe.get("stage2_noise_policy") or DEFAULT_STAGE2_NOISE_POLICY)
    if policy == "same_seed":
        return int(seed) & UINT64_MASK
    if policy == "fixed":
        try:
            return int(recipe.get("stage2_fixed_seed", DEFAULT_STAGE2_FIXED_SEED)) & UINT64_MASK
        except (TypeError, ValueError):
            return DEFAULT_STAGE2_FIXED_SEED
    return _stage2_seed(seed)
...
def _effective_recipe(request: dict) -> dict:
    ...
    common_sampler = normalize_sampler_name(raw.get("sampler")) or DEFAULT_SAMPLER
    stage1_sampler = normalize_sampler_name(raw.get("stage1_sampler")) or common_sampler or DEFAULT_STAGE1_SAMPLER
    stage2_sampler = normalize_sampler_name(raw.get("stage2_sampler")) or common_sampler or DEFAULT_STAGE2_SAMPLER
    ...
    video_cfg = bounded_float("video_cfg", bounded_float("cfg", DEFAULT_VIDEO_CFG, 0.0, 100.0), 0.0, 100.0)
    audio_cfg = bounded_float("audio_cfg", bounded_float("cfg", DEFAULT_AUDIO_CFG, 0.0, 100.0), 0.0, 100.0)
    user_guidance = request.get("guidance_scale")
    if user_guidance is not None:
        try:
            guidance = float(user_guidance)
        except (TypeError, ValueError):
            guidance = DEFAULT_CFG
        if not 0.0 <= guidance <= 20.0:
            guidance = DEFAULT_CFG
        # The public Guidance control intentionally remains one simple control.
        # When supplied, apply it to both AV CFG scales rather than silently
        # leaving audio on a different hidden value.
        video_cfg = guidance
        audio_cfg = guidance

    def strength(name: str, fallback: float) -> float:
        return bounded_float(name, fallback, 0.0, 1.0)

    stage2_noise_policy = str(raw.get("stage2_noise_policy") or DEFAULT_STAGE2_NOISE_POLICY).strip().lower()
    if stage2_noise_policy not in {"increment", "same_seed", "fixed"}:
        stage2_noise_policy = DEFAULT_STAGE2_NOISE_POLICY
    try:
        stage2_fixed_seed = int(raw.get("stage2_fixed_seed", DEFAULT_STAGE2_FIXED_SEED)) & UINT64_MASK
    except (TypeError, ValueError):
        stage2_fixed_seed = DEFAULT_STAGE2_FIXED_SEED

    negative_prompt = raw.get("negative_prompt", DEFAULT_NEGATIVE_PROMPT)
    ...
    return {
        "sampler": stage1_sampler if stage1_sampler == stage2_sampler else "mixed",
        ...
        "video_cfg": video_cfg,
        "audio_cfg": audio_cfg,
        "image_guide_strength": strength("image_guide_strength", DEFAULT_IMAGE_GUIDE_STRENGTH),
        "upscaled_image_guide_strength": strength(
            "upscaled_image_guide_strength", DEFAULT_UPSCALED_IMAGE_GUIDE_STRENGTH
        ),
        "negative_prompt": negative_prompt,
        "stage2_noise_policy": stage2_noise_policy,
        "stage2_fixed_seed": stage2_fixed_seed,
        "origin": origin,
    }
...
def _apply_guide_plan(nodes, *, positive, negative, latent, vae, image, guides):
    """Apply advanced first/last keyframe references for locked-shot mode only."""
    for guide in guides:
...
def _apply_inplace(nodes, *, latent, vae, image, strength: float):
    return _call_node(
        nodes,
...
def _dual_guider(nodes, *, model, positive, negative, recipe):
    return _call_node(
        nodes,
        "LTXVDualCFGGuider",
        model=model,
        positive=positive,
        negative=negative,
        video_cfg=float(recipe["video_cfg"]),
        audio_cfg=float(recipe["audio_cfg"]),
    )[0]
...
def _run(request: dict, output_dir: Path) -> dict:
    ...
    if profile_id != EXECUTION_PROFILE_ID:
        raise RuntimeError(
            f"Worker implements execution profile '{EXECUTION_PROFILE_ID}', not {profile_id or '<missing>'!r}"
        )
    ...
    stability_mode = normalize_i2v_stability(request.get("i2v_stability")) if input_image else MODEL_STABILITY
    if input_image:
        recipe, stability_overrides = apply_i2v_stability(source_recipe, stability_mode)
    else:
        recipe = dict(source_recipe)
        stability_overrides = {}
    stage2_seed = _resolved_stage2_seed(seed, recipe)

    locked_guides = input_image is not None and stability_mode == LOCKED_STABILITY
    stage1_guides = list(guide_plan(stability_mode, recipe["image_guide_strength"])) if locked_guides else []
    stage2_guides = list(guide_plan(stability_mode, recipe["upscaled_image_guide_strength"])) if locked_guides else []
    reference_conditioning = (
        "LTXVAddGuide"
        if locked_guides
        else ("LTXVImgToVideoInplace" if input_image else None)
    )

    print(
        "DuckMotion ConvRot execution recipe: "
        f"origin={recipe['origin']}; samplers={recipe['stage1_sampler']}/{recipe['stage2_sampler']}; "
        f"cfg(video/audio)={recipe['video_cfg']}/{recipe['audio_cfg']}; "
        f"i2v={recipe['image_guide_strength']}/{recipe['upscaled_image_guide_strength']}; "
        f"stage2_noise={recipe['stage2_noise_policy']}:{stage2_seed}; "
    ...
    negative = _call_node(nodes, "CLIPTextEncode", clip=clip, text=recipe["negative_prompt"])[0]
    ...
    stage1_sampler = _call_node(nodes, "KSamplerSelect", sampler_name=recipe["stage1_sampler"])[0]
    ...
    stage1 = _call_node(
        ...
        guider=_dual_guider(nodes, model=model, positive=positive, negative=negative, recipe=recipe),
    ...
    # Locked-shot references append guide tokens; only that advanced path needs
    # them cropped before the learned spatial upscaler. Normal I2V does not.
    stage2_positive, stage2_negative = positive, negative
    if locked_guides:
    ...
    # Native LTX-2.5 two-stage transition: learned x2 latent upscaling directly
    # into the refinement stage. Do NOT shrink it back with LatentUpscaleBy(0.5).
    video_latent = _call_node(
        nodes,
        "LTXVLatentUpsampler",
    ...
    stage2_sampler = _call_node(nodes, "KSamplerSelect", sampler_name=recipe["stage2_sampler"])[0]
    ...
    audio = _call_node(nodes, "LTXVAudioVAEDecode", samples=audio_latent, audio_vae=audio_vae)[0]
    ...
    meta = {
        ...
        "sampling": "ltx25_convrot_two_stage_av_native",
        "sampler": recipe["sampler"],
        "stage1_sampler": recipe["stage1_sampler"],
        "stage2_sampler": recipe["stage2_sampler"],
        "cfg": recipe["cfg"],
        "video_cfg": recipe["video_cfg"],
        "audio_cfg": recipe["audio_cfg"],
        "high_sigmas": recipe["stage1_sigmas"],
        "low_sigmas": recipe["stage2_sigmas"],
        "stage2_noise_policy": recipe["stage2_noise_policy"],
        "stage2_fixed_seed": recipe.get("stage2_fixed_seed"),
